...
@interface YCJGameRoomViewController ()<UICollectionViewDataSource,
...
@implementation YCJGameRoomViewController
...
- (void)requestRoomList {
    [JKNetWorkManager getRequestWithUrlPath:JKGameRoomListUrlKey parameters:@{@"categoryId": self.roomGroup.categoryId, @"groupId": self.roomGroup.roomGroupId} finished:^(JKNetWorkResult * _Nonnull result) {
        if (result.error) {
            ...
            self.roomList = [YCJRoomListModel mj_objectArrayWithKeyValuesArray:result.resultData[@"data"]];
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YCJGameRoomCell *cell =
            [YCJGameRoomCell cellWithCollectionView:collectionView
        ...
        didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /// 进入房间
    YCJRoomListModel *roomModel = self.roomList[indexPath.item];
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        //注册容器视图中显示的方块视图
        [_collectionView registerClass:[YCJGameRoomCell class]
                forCellWithReuseIdentifier:[YCJGameRoomCell cellIdentifier]];
        //注册容器视图中显示的顶部视图
