...
@interface YCJLoginViewController ()<ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding>
...
- (void)onekeyPhoneLogin {
    ...
    [JKNetWorkManager postRequestWithUrlPath:JKOneKeyLoginUrlKey parameters:[param copy] finished:^(JKNetWorkResult * _Nonnull result) {
        if (result.error) {
            ...
            /// 保存用户信息
            YCJUserInfoModel *userInfoModel = [YCJUserInfoModel mj_objectWithKeyValues:result.resultData];
            YCJToken *tokenModel = [YCJToken mj_objectWithKeyValues:result.resultData[@"accessToken"]];
            [YCJUserInfoManager sharedInstance].userInfoModel = userInfoModel;
            [YCJUserInfoManager sharedInstance].userTokenModel = tokenModel;
...
- (void)loginAction:(UIButton *)send {
    ...
    if (send.tag == 80) {
        ...
        YCJSMSLoginViewController *vc = [[YCJSMSLoginViewController alloc] init];
