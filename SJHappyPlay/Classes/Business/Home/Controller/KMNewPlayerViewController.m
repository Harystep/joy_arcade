...
@interface KMNewPlayerViewController ()
...
@implementation KMNewPlayerViewController
...
- (void)requestPlayList {
    [JKNetWorkManager getRequestWithUrlPath:JKGameIntroListUrlKey parameters:@{@"categoryId": @"0"} finished:^(JKNetWorkResult * _Nonnull result) {
        if (result.error) {
            ...
            self.playList = [KMNewPlayerModel mj_objectArrayWithKeyValuesArray:result.resultData];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    KMNewPlayerModel *playModel = self.playList[indexPath.row];
